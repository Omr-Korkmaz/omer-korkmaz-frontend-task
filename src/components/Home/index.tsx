...
// const Home: React.FC = () => {
//   const { data: swapiData, isLoading: swapiLoading, error: swapiError } = useFetchApi<swapiApiResponse<swapiApiParams>>(
//     "https://swapi.dev/api/films/?format=json"
//   );
...
//   const [selectedItem, setSelectedItem] = useState<SelectedItem | null>(null);
//   const [filter, setFilter] = useState<string>("");
//   const [sortingCriteria, setSortingCriteria] = useState<{ column: string; order: string }>({
//     column: "release_date",
//     order: "asc",
//   });
...
//   const handleItemClick = (itemId: number) => {
//     const selectedItem = swapiResult?.find((item: swapiApiParams) => item.episode_id === itemId);
//     setSelectedItem(selectedItem ? selectedItem : null);
//   };
...
//     const comparison = sortingCriteria.order === "asc" ? 1 : -1;
//     return a[sortingCriteria.column] > b[sortingCriteria.column] ? comparison : -comparison;
//   });
...
//         <Grid item xs={6} sx={{ height: "100vh", borderLeft: "1px solid gray" }}>
//           <div>
//             <MovieDetails selectedItem={selectedItem} />
//           </div>
//         </Grid>
...
const Home: React.FC = () => {
  ...
  const modifiedSwapiData = swapiResult?.map(item => ({
    ...item,
    fullName: `Episode ${convertLatinToRomanUtils(
      item?.episode_id
    )} - ${item?.title}`
  }));
  ...
  const handleItemClick = (itemId: number) => {
    const selectedItem = modifiedSwapiData?.find((item: swapiApiParams) => item.episode_id === itemId);
    setSelectedItem(selectedItem ? selectedItem : null);
  };
  ...
  const modifiedfilteredSwapiData = modifiedSwapiData ? filterUtils(modifiedSwapiData, filter) : [];

   const modifiedSortedSwapiData = [...modifiedfilteredSwapiData].sort((a, b) => {
    if (sortingCriteria.column === "release_date") {
      const dateA: any = parseDate(a[sortingCriteria.column]);
      const dateB: any = parseDate(b[sortingCriteria.column]);
      return sortingCriteria.order === "asc" ? dateA - dateB : dateB - dateA;
